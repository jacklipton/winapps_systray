...
type SettingsWindow struct {
	...
	snapshot config.Settings // copy of settings at window open time
	saved    bool            // true if user clicked Save
...
func (s *SettingsWindow) Show() {
	...
	win.SetDefaultSize(420, 480)
	win.SetPosition(gtk.WIN_POS_CENTER)
	s.window = win
	s.snapshot = *s.settings
	s.saved = false
	win.Connect("destroy", func() {
		if !s.saved {
			*s.settings = s.snapshot
		}
		s.window = nil
	})
	...
	notebook.Connect("switch-page", func(_ *gtk.Notebook, _ *gtk.Widget, pageNum uint) {
		if s.lblVMStatus != nil {
			s.lblVMStatus.SetText("")
		}
	})
...
func (s *SettingsWindow) buildAppSettingsTab() *gtk.Box {
	...
	adj, _ := gtk.AdjustmentNew(float64(s.settings.PollIntervalSeconds), 1, 300, 1, 5, 0)
